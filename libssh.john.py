...
class Connection(ConnectionBase):
    ...
    def _get_proxy_command(self, port=22):
        ...
        #raise AnsibleConnectionFailure('JOHN: ' + str(ssh_args))
        #raise AnsibleConnectionFailure('JOHN: ' + self.get_option("proxy_command"))
        if ssh_args is not None:
            
            #args = self._split_ssh_args(" ".join(ssh_args))

            temp = " ".join(ssh_args)
            expression = '(ProxyCommand)(=)(\")(.*)(\")'
            results = re.search(expression, temp)
            # raise AnsibleConnectionFailure('JOHN: ' + str(results.group(1)))
            if results:
                proxy_command = results.group(4)
            else:
                proxy_command = ""
            #raise AnsibleConnectionFailure('JOHN: ' + proxy_command)

            #raise AnsibleConnectionFailure('JOHN: ' + str(args))
        ...
        #raise AnsibleConnectionFailure('JOHN: ' + str(self._play_context.ssh_common_args))
        #proxy_command = 'ssh -i ~/.ssh/vagrant_rsa -W %h:%p -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -oProxyCommand=\"ssh -i ~/.ssh/vagrant_rsa -W lab.example.com:22 -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null vagrant@jh1.example.com\" vagrant@lab.example.com'
        
        # JOHN - Get option from ansible.cfg
        #proxy_command = self.get_option("proxy_command")

        #raise AnsibleConnectionFailure('JOHN: libssh - _get_proxy_command(): ' + proxy_command)
        if proxy_command:
            replacers = {
        ...
        #raise AnsibleConnectionFailure(proxy_command)
    ...
    def _connect_uncached(self):
        ...
        #raise AnsibleConnectionFailure('JOHN: ' + str(proxy_command))
        try:
            private_key = None
